...
const InterventionListA = () => {
  ...
  // Updated columns to include edit action
  const columns = [
    { 
      field: 'date', 
      headerName: 'Date', 
        ...
        <Box display="flex" gap={1}>
          ...
          <Tooltip title="Modifier l'intervention">
            <IconButton 
              size="small" 
              color="secondary"
              onClick={() => handleEditIntervention(params.row)}
            >
              <EditIcon />
            </IconButton>
          </Tooltip>
  ...
  useEffect(() => {
    const fetchInterventions = async () => {
      try {
        const response = await apiAccount.get("sadmin/list/interventions/", {
  ...
  const handleViewIntervention = (intervention) => {
    navigate(`/interventions/detailsA/${intervention.id}`);
  };

  // Edit intervention 
  const handleEditIntervention = (intervention) => {
    navigate(`/interventions/editA/${intervention.id}`);
  };
...
export default InterventionListA;